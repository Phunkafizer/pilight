...
static void *raspyrfmSend(int reason, void *param) {
...
static unsigned short raspyrfmHwInit(void) {
	...
	if(wiringXSPISetup(rfmsettings.spi_ch, 20000) == -1) {
...
void raspyrfmInit(void) {
	...
	options_add(&raspyrfm->options, 'c', "spi_ch", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_NUMBER, NULL, "^[01]$");
	options_add(&raspyrfm->options, 'b', "band", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_NUMBER, NULL, "^[01]$");
